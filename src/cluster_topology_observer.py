...
class ClusterTopologyObserver(Object):
    ...
    def start_observer(self) -> None:
        ...
        if not isinstance(self._charm.unit.status, ActiveStatus) or self._charm._peers is None:
            return
        if "observer-pid" in self._charm.state.peer.data:
            # Double check that the PID exists
            pid = int(self._charm.state.peer.data["observer-pid"])
            try:
                os.kill(pid, 0)
                return
            except OSError:
                pass
        if not self._charm.state.unit_ip:
        ...
        urls = [self._charm.state.patroni_url] + [
            self._charm.state.patroni_url.replace(self._charm.state.unit_ip, peer)
            for peer in list(self._charm._peer_members_ips)
        ]
        ...
        self._charm.state.peer.data.update({"observer-pid": f"{pid}"})
    ...
    def stop_observer(self):
        ...
        if self._charm._peers is None or "observer-pid" not in self._charm.state.peer.data:
            return

        if stored_pid := self._charm.state.peer.data.get("observer-pid"):
            ...
            try:
                ...
                self._charm.state.peer.data.update({"observer-pid": ""})
            except OSError:
                pass
