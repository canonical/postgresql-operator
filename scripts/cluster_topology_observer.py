...
def check_for_database_changes(run_cmd, unit, charm_dir, previous_databases_hash):
    ...
    try:
        ...
        current_databases_hash = subprocess.check_output(command, env=env)  # noqa: S603
    except subprocess.CalledProcessError as e:
        with open(LOG_FILE_PATH, "a") as log_file:
            log_file.write(f"Failed to retrieve databases: {e}\n")
        return previous_databases_hash
    else:
        # If it's the first time the databases were retrieved, then store it and use
        # it for subsequent checks.
        if not previous_databases_hash:
            previous_databases_hash = current_databases_hash
        # If the databases changed, dispatch a charm event to handle this change.
        elif current_databases_hash != previous_databases_hash:
            previous_databases_hash = current_databases_hash
