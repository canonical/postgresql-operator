...
def test_get_extensions(harness):
    ...
    tc().assertEqual(
        harness.charm.legacy_db_relation._get_extensions(relation), ([], set())
    )
...
def test_update_unit_status(harness):
    with (
        patch("relations.db.DbProvides._check_for_blocking_relations") as _check_for_blocking_relations,
        patch("charm.PostgresqlOperatorCharm.is_blocked", new_callable=PropertyMock) as _is_blocked,
    ):
    ...
    with (
        ...
        for rel in [rel_id, another_rel_id]:
            ...
            assert_based_on_relation = (
                tc().assertTrue if rel == rel_id else tc().assertFalse
            )
...
def test_get_allowed_units(harness):
    ...
    tc().assertEqual(
        harness.charm.legacy_db_relation._get_allowed_units(db_relation), "application/0 application/1"
    )
