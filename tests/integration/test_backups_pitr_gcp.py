...
async def pitr_backup_operations(
    ...
    async with ops_test.fast_forward():
        try:
            ...
            logger.info(
                f"!!!!!!!!!!!!!!!!!!!!!!!!!!! {await run_command_on_unit(ops_test, 'postgresql-gcp/0', 'tar czf /tmp/pglogs.tar.gz /var/snap/charmed-postgresql/common/var/log/')}"
            )
            logger.info(
                f"!!!!!!!!!!!!!!!!!!!!!!!!!!! {await ops_test.juju('scp', 'postgresql-gcp/0:/tmp/pglogs.tar.gz', '/home/runner/pglogs.tar.gz')}"
            )
            raise e
    logger.info(
