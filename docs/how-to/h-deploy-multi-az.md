...
Let's destroy GCE node and recreate it using the same AZ:
```shell
...
Here we should remove no-longer available server/vm/GCE node and add a new one, Juju will create it in the same AZ `us-east4-c`:
```shell
...
The command `juju status` shows the healthy machines state, but PostgreSQL HA recovery necessary:
```shell
Model    Controller  Cloud/Region     Version    SLA          Timestamp
...
Re-check no running GCE instances left (it should be empty):
```shell
gcloud compute instances list
```