...
run "defaults_reach_the_application" {
  ...
  assert {
    condition     = juju_application.machine_postgresql.charm[0].channel == "16/stable"
    error_message = "channel must default to \"16/stable\""
  }

  assert {
    condition     = juju_application.machine_postgresql.charm[0].base == "ubuntu@24.04"
...
run "config_and_storage_are_passed_through" {
  ...
  assert {
    condition     = juju_application.machine_postgresql.charm[0].channel == "16/edge"
  ...
  assert {
    condition     = juju_application.machine_postgresql.storage_directives["pgdata"] == "10G"
